...
module.exports = {
        ...
        try {
            ...
            if (!user) {
                ...
                userSchema.create(userDetails).then((newUser) => {
                    ...
                    const accessToken = jwt.sign(user, "myWebAppSecretKey123", { expiresIn: '1d' })
    ...
    apiVerification: async (req, res) => {
       ...
       try{
        ...
        jwt.verify(token, "myWebAppSecretKey123", async (err, data) => {
            if (err) {
                res.json({
                    status: false
                })
            ...
            if (isUser) {
                ...
                if (password == false) {
                    ...
                    if (isUser.status === false) {
                        ...
                        //jwt authentication of user---------------------------
                        const accessToken = jwt.sign(userIs, "myWebAppSecretKey123", { expiresIn: '1d' })
